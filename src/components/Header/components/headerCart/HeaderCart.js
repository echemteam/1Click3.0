...
const HeaderCart = ({ closemodal }) => {
  ...
  const [updateShoppingCartById, { isLoading: isUpdateLoading, isSuccess: isUpdateSuccess }] = useUpdateShoppingCartByIdMutation();
  ...
  useEffect(() => {
    if (!isGetShoppingCartByIdLoading && isGetShoppingCartByIdSuccess && isGetShoppingCartByIdData) {
  ...
  const handleremoveproductfromcart = (shopppingCartProductId) => {
    shopppingCartProductId = shopppingCartProductId.toString().trim();
    confirm(
      "Remove Product?",
      ...
      if (result) {
        const request = {
          ShoppingCartProductIds: shopppingCartProductId,
        };
  ...
  return (
    <div className="shopping-cart-section">
      {(isGetShoppingCartByIdLoading || isUpdateLoading) && <DataLoader />}

      {/* Cart list */}
      {!isGetShoppingCartByIdLoading &&
        !isUpdateLoading &&
        isGetShoppingCartByIdSuccess &&
        shoppingCartData.length > 0 && (
          <>
            <div className="title-desc-top">
              <div className="left-title">{shoppingCartData.length} Items</div>
              <button className="right-desc" onClick={handleviewcart}>
                View Cart
              </button>
            </div>
            ...
            <div className="product-lists">
              {shoppingCartData.map((product, index) => (
                <div
                  className="cart-lists-item"
                  key={product.shopppingCartProductId || index}
                >
                  <div className="left-section">
                    <div className="product-item-image-container">
                      <Image
                        src={AppIcons.ProductDummyImage}
                        alt="Product"
                        width={100}
                        height={100}
                    ...
                    <div className="product-status">
                      <div className="product-status-tag red">
                        ...
                        <div className="product-status-title">Out of Stock</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="middle-section">
                    ...
                    <div className="middle">
                      <div className="product-details">
                        <div className="product-title">Catalog :</div>
                        <div className="product-value">{product.catalogId}</div>
                      </div>
                      <div className="product-details">
                        <div className="product-title">Cas No :</div>
                        <div className="product-value">{product.casNo}</div>
                      </div>
                      <div className="product-details">
                        <div className="product-title">MDL No :</div>
                        <div className="product-value">{product.mdlNo}</div>
                      </div>
                    </div>
            ...
            <div className="cart-summary">
              <div className="left-summary">
                <div className="summary-label">No. of Products :</div>
                <div className="summary-value">{shoppingCartData.length}</div>
              </div>
            ...
            <div className="bottom-navigation">
              <Button
                variant="contained"
                color="secondary"
                endIcon="solar:arrow-right-outline"
