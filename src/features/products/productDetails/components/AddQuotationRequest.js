...
const AddQuotationRequest = ({ onClose, catalogNumber, selectedProducts }) => {
  ...
  const [
    addRfq,
    {
      isLoading: isAddRfqLoading,
      isSuccess: isAddRfqSuccess,
      data: isAddRfqData,
    },
  ] = useAddRfqMutation();
  const [
  ...
  const addProductRow = (e) => {
    ...
    const isPackageValid = validateField(
      "package",
      currentProduct.package,
      currentIndex
    );
    const isQuantityValid = validateField(
      "quantity",
