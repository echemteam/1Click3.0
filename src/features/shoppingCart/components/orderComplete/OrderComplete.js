...
const OrderComplete = () => {
  ...
  useEffect(() => {
    ...
    if (orderId > 0 && shoppingCartListData.length > 0) {
      const allids = shoppingCartListData
        .map((item) => item.shopppingCartProductId)
        .toString()
        .trim();
      const request = { ShoppingCartProductIds: allids };
      updateShoppingCartById(request);
      calledRef.current = true;
    }
  }, [orderId, shoppingCartListData, updateShoppingCartById]);
